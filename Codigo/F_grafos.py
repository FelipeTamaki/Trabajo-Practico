...
def KPI(origen, destino, carga): #Calcula ambas soluciones optimas (costo y tiempo), imprime el medio de transporte, el camino, el costo y el tiempo que tarda. Al mis
    camion = Automotor()
    tren = Ferroviario()
    barco = Fluvial()
    avion = Aereo()
...
def graficar(titulo,tiempo_acumulado,distancia_acumulada,costo_acumulado, grafo_lista, camino_minimo, tipo_minimo,costo_minimo,tiempo_minimo):
    ...
    if 'tiempo' in titulo.lower(): # Esto es para que solo lo haga cuando hacemos el grafo de tiempo
        edge_labels = {c: str(timedelta(hours=valor)) for c, valor in conexiones.items()}
    else: # Si no tiene tiempo en el titulo lo mostramos como viene
        edge_labels = {c: f"{valor}" for c, valor in conexiones.items()}

    nx.draw_networkx_edge_labels(
        grafo, ordenamiento, edge_labels=edge_labels, ax=ax, font_color='black')
